...
- **Live Application URL**: [https://hypermindz-analytics.vercel.app](https://hypermindz-analytics.vercel.app) *(Mock URL for illustration — please update with your live link)*
- **Demo Credentials**:
  - **Email**: `demo@hypermindz.com`
  - **Password**: `password123` *(Automatically seeded with a sample sales database)*
...
graph TD
    ...
    B -->|Schema Introspection| C[Google Gemini 2.5 Flash]
...
- **SQLite3**: Serves as both the user-file database (physical isolation) and central metadata/logging registry.
...
- **`users`**: `id` (UUID), `email` (Unique), `password_hash` (Bcrypt).
...
sequenceDiagram
    ...
    DB->>BE: Row Records
    BE->>User: JSON Payload (Data + SQL + Viz + Explanation)
```
...
2. **Stateful Chat Context**: The last 10 messages of the chat thread are formatted into the system prompt to let the LLM resolve follow-up phrases (e.g. "filter that for West").
...
*Note: Register an account or log in to `demo@hypermindz.com` (which comes pre-seeded with the Sales Dataset) to try these:*
...
3. **"Show top 5 products by sales revenue"**
   ...
   - **Expected Result**: A table of 5 rows showing high-value products like "Wireless Headphones" or "Air Fryer" with the highest revenue.
...
*Note: In the frontend Settings screen, users can paste their own Google Gemini API key, which gets saved in local storage and sent via the `X-Gemini-Key` header, bypassing the server environment variable.*
...
```bash
git clone https://github.com/your-username/hypermindz-analytics-engine.git
...
1. **Dockerize** the FastAPI app (we provide a lightweight Dockerfile) or deploy directly as a Python service.
2. Set Environment Variables: `GEMINI_API_KEY` and `JWT_SECRET_KEY`.
3. SQLite databases are stored in the `backend/` folder on disk. For persistent storage on Render/Railway, attach a **Persistent Disk Volume** mapped to `/app/backend` (or wherever sqlite databases are stored) to prevent data loss on container rebuilds.

### Frontend Deployment (Vercel)
1. Import the `frontend/` subdirectory in Vercel.
2. Configure build settings: Framework preset `Next.js`.
3. Set the environment API base URL if hosting on a custom domain.
